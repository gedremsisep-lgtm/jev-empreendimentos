   ...
   <div class="fr3">
     <div class="fg"><label>Conjunto</label>
       <input id="ob-conj" list="ob-conjs" value="${esc(o?(o.conjunto||''):'')}" placeholder="Deixe vazio se for obra avulsa"
         ${o?'':'onchange="obraPuxarConjunto()" oninput="obraPuxarConjunto()"'}>
       <datalist id="ob-conjs">${conjuntos.map(c=>`<option value="${esc(c)}">`).join('')}</datalist>
       <div class="hint">${o ? 'Liga esta casa às outras do mesmo grupo, sem virar condomínio.'
         : 'Escolha um conjunto que já existe e o sistema <b>copia os dados da última casa lançada</b>, já sugerindo o próximo nome.'}</div></div>
...
/* ---------------- próxima casa de um conjunto ----------------
   Ao escolher um conjunto que já existe, o formulário se preenche com os
   dados da última casa lançada e sugere o nome seguinte: Casa 1 → Casa 2. */

/* "Casa 1"→"Casa 2" · "Casa 03"→"Casa 04" · "Vila X — Casa 09"→"Vila X — Casa 10" */
function proximoNome(nome){
  const m = String(nome||'').match(/^(.*?)(\d+)(\D*)$/);
  if(!m) return '';
  const n = String(parseInt(m[2],10)+1);
  return m[1] + (m[2].length > n.length ? n.padStart(m[2].length,'0') : n) + m[3];
}
/* o pedaço final que numera a casa, para trocar também no endereço */
function sufixoCasa(nome){
  const m = String(nome||'').match(/([^\s—–-]+\s*\d+)\s*$/);
  return m ? m[1].trim() : '';
}

let OB_PUXOU = '';        // evita repuxar a cada tecla digitada

async function obraPuxarConjunto(){
  const campo = document.getElementById('ob-conj');
  if(!campo) return;
  const conj = campo.value.trim();
  if(!conj || conj === OB_PUXOU) return;

  const irmas = (await dbGetAll('obras')).filter(x=>(x.conjunto||'') === conj);
  if(!irmas.length) return;                     // conjunto novo: nada a copiar
  OB_PUXOU = conj;

  /* a última lançada é a de id mais alto */
  const base = irmas.sort((a,b)=>Number(a.id)-Number(b.id))[irmas.length-1];
  const vazio = id => { const e=document.getElementById(id); return !e || !String(e.value||'').trim(); };
  const zero  = id => { const e=document.getElementById(id); return !e || !num(e.value); };
  const por   = (id,v) => { const e=document.getElementById(id); if(e && v!=null && v!=='') e.value = v; };

  /* nome: o próximo da sequência, pulando o que já existir */
  let novo = proximoNome(base.nome);
  if(novo){
    const usados = new Set(irmas.map(x=>String(x.nome||'').trim().toLowerCase()));
    let voltas = 0;
    while(usados.has(novo.trim().toLowerCase()) && voltas++ < 60) novo = proximoNome(novo);
    if(vazio('ob-nome')) por('ob-nome', novo);
  }

  /* dados que valem para o conjunto inteiro */
  por('ob-tipo', base.tipo);
  if(vazio('ob-resp')) por('ob-resp', base.respTec);
  if(vazio('ob-cli'))  por('ob-cli',  base.cliente);
  if(vazio('ob-tel'))  por('ob-tel',  base.telefone);
  if(vazio('ob-cid'))  por('ob-cid',  base.cidade);

  /* endereço: troca só o pedaço que numera a casa ("… — Casa 03" → "… — Casa 04") */
  if(vazio('ob-end') && base.endereco){
    const de = sufixoCasa(base.nome), para = sufixoCasa(novo);
    let end = base.endereco;
    if(de && para && end.trim().endsWith(de)) end = end.trim().slice(0, -de.length) + para;
    por('ob-end', end);
  }

  /* planta e números */
  por('ob-modelo', base.modelo || '');
  const sel = document.getElementById('ob-var'); if(sel) sel.value = base.variacao || '';
  if(zero('ob-area'))    por('ob-area',    base.area || '');
  if(zero('ob-terreno')) por('ob-terreno', base.terreno || '');
  if(zero('ob-pav'))     por('ob-pav',     base.pavimentos || 1);
  if(zero('ob-vc'))      por('ob-vc',      base.valorContrato || '');

  /* mantém o início escolhido e repete o prazo que a casa anterior teve */
  const dias = (base.dtInicio && base.dtPrevFim) ? diasEntre(base.dtInicio, base.dtPrevFim) : 0;
  if(dias > 0) por('ob-fim', addDias(val('ob-ini') || hoje(), dias));

  toast(`Dados copiados de "${base.nome}". Confira e ajuste o que mudar nesta casa.`);
}
...
async function conjuntoForm(){
  ...
  CONJ = {
    ...
    casas: [],
    existentes: [],                                  // obras antigas trazidas para o conjunto
    disponiveis: obras.sort((a,b)=>String(a.cod||'').localeCompare(String(b.cod||''))),
    busca: ''
  };
  const b = `
   ...
   <div class="divider"></div>
   <div class="sh"><i class="ti ti-building-community"></i>Obras já cadastradas
     <span class="sp"></span><span class="tt">opcional</span></div>
   <div class="tt" style="margin-bottom:9px">Se algumas casas do conjunto já estão no sistema, marque-as aqui em vez
     de cadastrar de novo. Elas entram para o conjunto do jeito que estão — <b>nome, área, valor, cronograma e
     orçamento continuam os mesmos</b>. Nada é recriado nem apagado.</div>
   <div id="cj-existentes"></div>

   <div class="divider"></div>
   <div class="sh"><i class="ti ti-list-numbers"></i>Casas novas</div>
  ...
  conjuntoExistentesRender();
  conjuntoGerarLista();
}

/* ---------------- obras já cadastradas ---------------- */
function conjuntoExistentesRender(){
  const box = document.getElementById('cj-existentes');
  if(!box) return;
  const busca = (CONJ.busca||'').toLowerCase();
  const lista = CONJ.disponiveis.filter(o=>!busca ||
    (String(o.cod||'')+' '+String(o.nome||'')+' '+String(o.conjunto||'')+' '+String(o.cliente||''))
      .toLowerCase().includes(busca));
  const marcada = id => CONJ.existentes.find(x=>Number(x.id)===Number(id));

  if(!CONJ.disponiveis.length){
    box.innerHTML = `<div class="al ai"><i class="ti ti-info-circle"></i>Ainda não há obras cadastradas para trazer —
      monte as casas novas ali embaixo.</div>`;
    return;
  }

  box.innerHTML = `
    <div class="fr3" style="align-items:end;margin-bottom:9px">
      <div class="fg" style="grid-column:span 2"><label>Procurar obra</label>
        <input id="cj-busca" value="${esc(CONJ.busca||'')}" placeholder="Código, nome, cliente ou conjunto"
          oninput="CONJ.busca=this.value;conjuntoExistentesRender();document.getElementById('cj-busca').focus()"></div>
      <div class="fg"><div class="tt">${CONJ.existentes.length} de ${CONJ.disponiveis.length} marcada(s)</div>
        ${CONJ.existentes.length?`<button class="btn gh sm blk" onclick="conjuntoLimparExistentes()">
          <i class="ti ti-square-off"></i>Desmarcar todas</button>`:''}</div>
    </div>
    <div class="card tw"><div class="bd" style="max-height:290px;overflow:auto"><table>
      <thead><tr><th class="c" style="width:38px"></th><th>Obra</th><th>Situação</th>
        <th class="r">Área</th><th class="r">Contrato</th><th>Papel no conjunto</th></tr></thead><tbody>
      ${lista.length ? lista.map(o=>{
        const m = marcada(o.id);
        const v = m ? VARIACOES[m.variacao] : null;
        return `<tr style="${m?'background:var(--brand-bg)':''}">
          <td class="c"><input type="checkbox" ${m?'checked':''} style="width:17px;height:17px;cursor:pointer"
            onchange="conjuntoTogglarObra(${o.id})"></td>
          <td><b class="mono">${esc(o.cod||'—')}</b> ${esc(o.nome||'sem nome')}
            ${o.conjunto?`<div class="tt">hoje no conjunto <b>${esc(o.conjunto)}</b></div>`:''}</td>
          <td><span class="st ${ST_OBRA[o.status]||'s-nt'}">${esc(o.status||'—')}</span></td>
          <td class="r n">${qtd(num(o.area),2)} m²</td>
          <td class="r n">${moeda(num(o.valorContrato))}</td>
          <td>${m ? `<select onchange="conjuntoExistenteVar(${o.id},this.value)"
                  style="border:1px solid var(--border2);border-radius:6px;padding:4px 7px;color:${v.cor};font-weight:700">
                  ${Object.keys(VARIACOES).map(k=>`<option ${m.variacao===k?'selected':''}>${k}</option>`).join('')}
                </select>` : '<span class="tt">—</span>'}</td>
        </tr>`;
      }).join('') : `<tr><td colspan="6" class="c tt" style="padding:18px">Nenhuma obra encontrada com esse texto.</td></tr>`}
      </tbody></table></div></div>`;
}
function conjuntoTogglarObra(id){
  const i = CONJ.existentes.findIndex(x=>Number(x.id)===Number(id));
  if(i>=0){ CONJ.existentes.splice(i,1); }
  else{
    const o = CONJ.disponiveis.find(x=>Number(x.id)===Number(id));
    if(o) CONJ.existentes.push({id:o.id, nome:o.nome, area:num(o.area), valor:num(o.valorContrato),
                                variacao:'Individual'});
  }
  conjuntoExistentesRender(); conjuntoCasasRender();
}
function conjuntoExistenteVar(id, variacao){
  const x = CONJ.existentes.find(y=>Number(y.id)===Number(id));
  if(x) x.variacao = variacao;
  conjuntoExistentesRender();
}
function conjuntoLimparExistentes(){
  CONJ.existentes = [];
  conjuntoExistentesRender(); conjuntoCasasRender();
}
...
function conjuntoCasasRender(){
  ...
  if(!box) return;
  const t0 = conjuntoTotais();
  if(!CONJ.casas.length){
    box.innerHTML = t0.trazidas
      ? `<div class="al ai"><i class="ti ti-info-circle"></i><div>Nenhuma casa nova — o conjunto vai ser formado
          só pelas <b>${t0.trazidas} obra(s) já cadastrada(s)</b> que você marcou acima, somando
          <b>${qtd(t0.area,2)} m²</b> e <b>${moeda(t0.valor)}</b> de contrato. Se quiser acrescentar casas novas,
          preencha a quantidade e clique em “Montar a lista”.</div></div>`
      : '';
    return;
  }
  ...
  box.innerHTML = `
    ...
    <div class="card tw"><div class="bd"><table>
      ...
      <tfoot><tr><td>${CONJ.casas.length} casa(s) nova(s)${t0.trazidas?` + ${t0.trazidas} já cadastrada(s)`:''}</td>
        <td colspan="2"></td>
        <td class="r n">${qtd(t0.area,2)} m²</td>
        <td class="r n">${moeda(t0.valor)}</td></tr></tfoot>
    </table></div></div>`;
}
function conjuntoTotais(){
  ...
  (CONJ.existentes||[]).forEach(e=>{ area += num(e.area); valor += num(e.valor); });
  return {area, valor, novas: CONJ.casas.length, trazidas: (CONJ.existentes||[]).length};
}
...
async function conjuntoSave(){
  ...
  const trazidas = CONJ.existentes || [];
  if(!conjunto) return falha('Dê um nome ao conjunto — é ele que liga as casas umas às outras.');
  if(!CONJ.casas.length && !trazidas.length)
    return falha('Marque alguma obra já cadastrada, ou monte a lista das casas novas, antes de salvar.');
  if(CONJ.casas.length && !ini) return falha('Informe a data de início da primeira casa.');
  ...
  err.innerHTML = `<div class="al ai"><i class="ti ti-loader-2"></i>${
    CONJ.casas.length ? 'Criando as casas…' : 'Montando o conjunto…'}</div>`;
  ...
  /* obras que já existiam: entram para o conjunto sem serem recriadas.
     Nada de cronograma, orçamento, nome ou valor é tocado.            */
  let mudadas = 0;
  for(const t of trazidas){
    const o = await dbGet('obras', t.id);
    if(!o) continue;
    await dbPut('obras', {...o,
      conjunto,
      variacao: t.variacao || 'Individual',
      espelhada: t.variacao === 'Espelhada',
      modelo: o.modelo || ''
    });
    mudadas++;
  }

  OBRA = criadas[0] || (trazidas[0] && trazidas[0].id) || OBRA;
  closeModal('mk-form');
  await refreshObraSelect();
  const partes = [];
  if(criadas.length) partes.push(`${criadas.length} casa(s) criada(s)`);
  if(mudadas)        partes.push(`${mudadas} obra(s) já cadastrada(s) trazida(s)`);
  toast(`${partes.join(' e ')} no conjunto ${conjunto}.`);
  go('obras');
}
